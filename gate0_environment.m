...
n_verified=0; n_mat_warn=0; n_missing=0; n_fail=0;
ctx.manifest = [];
...
if isfile(manifest_path)
    ...
    n_total_accounted = n_verified + n_missing + n_fail;
    ...
    total_accounted = n_verified + n_missing + n_fail;
    fprintf('    Manifest: %d entries | VERIFIED=%d (incl. %d MAT warns) | MISSING=%d | HASH_MISMATCH=%d\n',...
        height(mf), n_verified, n_mat_warn, n_missing, n_fail);
    fprintf('    Arithmetic check: %d+%d+%d = %d (should equal %d entries)\n',...
        n_verified, n_missing, n_fail, total_accounted, height(mf));
    ...
    if n_total_accounted ~= height(mf)
        error('BLOCKED_MANIFEST_ACCOUNTING_ERROR: %d + %d + %d = %d != %d manifest entries',...
            n_verified, n_missing, n_fail, n_total_accounted, height(mf));
    end
    if n_fail > 0
    ...
    ctx.artifact_status = sprintf('VERIFIED_%d_of_%d_MISSING_%d_MAT_WARN_%d', ...
        n_verified, height(mf), n_missing, n_mat_warn);
else
    ...
    if ~isempty(ctx.predictions)
        ...
        if ~isempty(pop_col) && ~isempty(pred_col)
            ...
            if numel(ym)==ctx.n_popA && all(isfinite(yp))
                ...
                if abs(r2_check-(-2.6162)) < ctx.tol
                    ctx.prediction_reproduced = true;
                    fprintf('    Canonical Pop-A R²=%.4f: REPRODUCED\n', r2_check);
                else
                    fprintf('    WARNING: Pop-A R²=%.4f vs canonical -2.6162 (diff=%.4e)\n',...
                        r2_check, abs(r2_check+2.6162));
                end
            end
        end
    end
...
T_env = table(...
    string({'matlab_version','n_manifest_verified','n_mat_warn','n_hash_fail',...
    ...
    string({ctx.matlab_version, num2str(n_verified), num2str(n_mat_warn),...
            num2str(n_fail), num2str(ctx.prediction_reproduced),...
...
elseif n_fail > 0
    ...
    g0_msg = sprintf('Required artifacts loaded; manifest: %d/%d verified, %d missing (intermediate files), %d MAT warns', ...
        n_verified, height(mf), n_missing, n_mat_warn);
end
if ~ctx.prediction_reproduced
...
ctx.gate_result = struct('status',string(g0_status),'code',string(g0_code),...
    ...
    'evidence_path',string(out_dir),'n_checks',n_verified+n_missing+n_fail,...
    'n_pass',n_verified);
