...
[y_raw,pred_audit] = nrr_eval.predict_frozen_deployment(run.deploy, T_B, cfg);
assert(pred_audit.N_RETAINED_CROSS_SEGMENT==0, ...
    '[G12] cross-segment blind prediction window retained');
...
writetable(pred_audit,fullfile(out4,'PRIMARY_BLIND_CNN_PREDICTION_AUDIT.csv'));
...
nrr_eval.write_eval(mA,'ridge_stacker',run.provenance,...
...
writetable(table({'ridge_stacker'},{run.provenance},...
